...
int main(int argc, char** argv) {
        ...
        std::shared_ptr<const Config> config = std::make_shared<const Config>(db_user, db_pass, db_host, db_name, db_migrations, server_pubkey, client_privkey, server_host, server_port);
        if(!config->valid()) {
