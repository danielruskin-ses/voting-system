...
AuditServerDatabase::AuditServerDatabase(const std::string& databasePath, const Logger& logger) : Database(databasePath, logger) {
}

ElectionMetadata AuditServerDatabase::fetchElectionMetadata() {
...
std::string AuditServerDatabase::fetchVoteServerPublicKey() {
        std::lock_guard<std::mutex> guard(_mutex);
...
std::string AuditServerDatabase::fetchAuditServerPublicKey() {
        std::lock_guard<std::mutex> guard(_mutex);

        // Execute query
        sqlite3_stmt* query = startQuery("SELECT AUDIT_SERVER_PUBLIC_KEY FROM CONFIG ORDER BY ID DESC LIMIT 1");
        bool res = executeQuery(query);
        if(!res) {
                finalizeQuery(query);
                throw std::runtime_error("No rows returned!");
...
std::string AuditServerDatabase::fetchAuditServerPrivateKey() {
        std::lock_guard<std::mutex> guard(_mutex);

        // Execute query
        sqlite3_stmt* query = startQuery("SELECT AUDIT_SERVER_PRIVATE_KEY FROM CONFIG ORDER BY ID DESC LIMIT 1");
        bool res = executeQuery(query);
        if(!res) {
                finalizeQuery(query);
                throw std::runtime_error("No rows returned!");
...
void AuditServerDatabase::saveConfig(int id, const ElectionMetadata& metadata, const std::string& voteServerPubKey, const std::string& auditServerPubKey, const std::string& auditServerPrivKey) {
        std::lock_guard<std::mutex> guard(_mutex);
...
std::string AuditServerDatabase::fetchVoterDevicePublicKey(int voterDeviceId) {
        std::lock_guard<std::mutex> guard(_mutex);
...
void AuditServerDatabase::saveVoterDevicePublicKey(int voterDeviceId, const std::string& pubKey) {
        std::lock_guard<std::mutex> guard(_mutex);
...
std::vector<SignedRecordedBallot> AuditServerDatabase::fetchSignedRecordedBallotsSorted() {
        std::lock_guard<std::mutex> guard(_mutex);
...
void AuditServerDatabase::saveSignedRecordedBallot(int voterDeviceId, const SignedRecordedBallot& signedRecordedBallot) {
        std::lock_guard<std::mutex> guard(_mutex);
...
bool AuditServerDatabase::isSignedTreeReceived() {
        std::lock_guard<std::mutex> guard(_mutex);
...
SignedTree AuditServerDatabase::fetchSignedTree() {
        std::lock_guard<std::mutex> guard(_mutex);
...
void AuditServerDatabase::saveSignedTree(int id, const SignedTree& signedTree) {
        std::lock_guard<std::mutex> guard(_mutex);
